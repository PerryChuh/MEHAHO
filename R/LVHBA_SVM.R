#' Support Vector Machine HO Based on LVHBA
...
#' @param c_0 which is used to generate the penalty parameter \eqn{c_k} in LV-HBA by \eqn{c_k = \underline{c}(k+1)^p} where \eqn{\underline{c} = \text{c_0}}.
#'     Default is 2.
...
#'   \item{Fseq}{The upper-level objective function value sequence generated by the iteration based on validation set.}
#' @export
...
LVHBA_SVM <- function(features,
  ...
  main_fun = function(features,
    ...
    for (i in 1:(K - 1)) {
      ...
      # 无放回的抽样，选取第i个训练集的索引集
      Omega_tr[[i]] = data_index[tr_set_indices] # 第i个训练集样本索引集
    ...
    for (t in 1:K) {
      ...
      A_t4 = cbind(matrix(0, nrow = nrow(E_xi[[t]]), ncol = p), matrix(0, nrow = nrow(E_xi[[t]]), ncol = 1), -E_xi[[t]]) # A_t第4行块
    ...
    up_fun_seq = numeric(N + 1)  # Array to record the upper-level objective values for each iteration
    X_seq = numeric(N)  # Array to record the upper-level variables for each iteration
    Y_seq = numeric(N)  # Array to record the lower-level variables for each iteration
    Theta_seq = numeric(N)  # Array to record the auxiliary variables for each iteration
    up_fun_seq[1] = up_fun_real(lambda, w_bar, w, c, xi)  # Initial upper-level objective value
    ...
    return(list(x = x, y = y, w = w, c = c, xi = xi, theta = theta, Xconv = X_seq, Yconv = Y_seq,
                Thetaconv = Theta_seq, up_fun_seq = up_fun_seq, val_error = val_error))
  }
  ...
  if(auto_tuning == TRUE){
    ...
    main_fun(features, labels, K, N, eta, alpha, beta, gamma1 = gamma1, gamma2 = gamma2, c_0 = c_0, r = r, ub = ub, lb = lb, tol = tol
    )
  }
}
