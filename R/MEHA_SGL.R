...
#' @param c \eqn{\underline{c}} in MEHA, which is used to generate the penalty parameter \code{c_k} in LV-HBA by \code{c_k = \underline{c}(k+1)^power}.
#'     Default is 1.
...
#' sigma <- 0.5*norm(a %*% true_beta, type = "2") / norm(epsilon, type = "2")
#' b <- a %*% true_beta + sigma * epsilon
...
#'
#' plot(result$F_seq, xlab = "iteration", type = "l",
