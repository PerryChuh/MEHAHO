...
#'     Here, we consider a generalized version of sparse group lasso by considering individual penalty parameters for each group lasso penalty.
...
#'     specific number of features in each group. For example, if the coefficients of dimension 12 are divided into 3 groups which is 6, 3, and 3 dimensions,
...
#' @param alpha Projection stepsize of \code{x}. Default is 1e-5 which is fixed.
...
#' @param beta Proximal gradient stepsize of \code{y}. Default is 1e-5 which is fixed.
#'     Specifically, you can edit the code and utilize the inverse power learning rate annealing strategy to dynamically adjust it.
#' @param eta Proximal gradient stepsize of the proxima \code{theta}.
#'     Default is 1e-5 which is fixed.
#' @param gamma Moreau envelope parameter. Default is 1.
#' @param c \eqn{\underline{c}} in MEHA, which is used to generate the penalty parameter \code{c_k} in LV-HBA by \code{c_k = \underline{c}(k+1)^p}.
#'     Default is 1.
#' @param p The index in \code{c_k = \underline{c}(k+1)^p}. Default is 0.48.
...
#'   \item{theta}{Auxiliary variable, which is of the same scale as variable y}
...
MEHA_SGL = function(A_val, b_val, A_tr, b_tr, group, N = 500, alpha = 1e-4,
                    beta = 1e-3, eta = 1e-3, gamma = 1, c = 1, p = 0.49,tol = 0.2,
  ...
  main_fun <- function(A_val, b_val, A_tr, b_tr, group,
                       N, alpha, beta, eta, gamma, c, p,tol){
  ...
  if(auto_tuning == TRUE){
    ...
    result <- main_fun(A_val, b_val, A_tr, b_tr, group,N, alpha = alpha.seq[1], beta = beta.seq[1], eta = eta.seq[1], c = c, gamma = gamma, p = p, tol = tol)
    ...
    for (j in 2:iter) {
      ...
      result <- main_fun(A_val, b_val, A_tr, b_tr, group,N, alpha = alpha.seq[j], beta = beta.seq[j], eta = eta.seq[j], c = c, gamma = gamma, p = p, tol = tol)
    ...
    return(main_fun(A_val, b_val, A_tr, b_tr, group, N, alpha = alpha.seq[opt_index], beta = beta.seq[opt_index], eta = eta.seq[opt_index], gamma, c, p, tol))

  }

  else{
    return(main_fun(A_val, b_val, A_tr, b_tr, group, N, alpha, beta, eta, gamma, c, p, tol))
