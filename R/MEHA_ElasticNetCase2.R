...
#'     elastic net based on the input training and validation sets using MEHA. In this case, the 2-norm term (which is smooth) is divided into \eqn{g(x,y)} in the problem studied by MEHA.
...
MEHA_ElasticnetCase2 = function(A_val, b_val, A_tr, b_tr, N = 500, alpha = 1e-3, beta = 1e-5, eta = 1e-5, gamma = 2, c = 1e-2, p = 0.1, tol = 0.05, auto_tuning = FALSE, temperature = 0.1){
  ...
  main_fun <- function(A_val, b_val, A_tr, b_tr, N, alpha, beta, eta, gamma = gamma, c = c, p = p, tol = tol){
    ...
    # LF = sum(A_val %*% t(A_val))
    # Lf = sum(A_tr %*% t(A_tr))
    # Lg = 1000
    # eta = (1 / gamma) / ((Lf + 1 / gamma) ** 2)
    # alpha = 1/(2*(LF/c+Lf+Lg))
    # beta = 1/(2*(LF/c+Lf))
